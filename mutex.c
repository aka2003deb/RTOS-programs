...
//OS_SEM semaphore;                      /* Semaphore to control  GLCD access*/
...
__task void lcd (void) {
  ...
  for (;;) {
   // os_sem_wait (&semaphore, 0xffff);
    ...
    GLCD_DisplayString(2, 0, __FI, "      Mut Demo      ");  //Critical section 
    //os_sem_send(&semaphore);
		os_mut_release(&mutex);
    os_dly_wait (100);
 ...
 //   os_sem_wait (&semaphore, 0xffff);
    ...
    GLCD_DisplayString(0, 0, __FI, "       ECE          ");
    GLCD_DisplayString(1, 0, __FI, "       RVCE         ");
    GLCD_DisplayString(2, 0, __FI, "      Mut Demo      ");
   // os_sem_send(&semaphore);
		os_mut_release(&mutex);
    os_dly_wait (100);
  }
